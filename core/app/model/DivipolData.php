<?php
class DivipolData{
    public static $Tablename = "divipol";
    ...
    public function Add(){
        $sql = "insert into ".self::$Tablename."(idDepartamento,idCiudad,nombre,referencia,comentario)";
        $sql .= "value(\"$this->idDepartamento\",\"$this->idCiudad\",\"$this->nombre\",\"$this->referencia\",\"$this->comentario\")";
        $query = Executor::doit($sql);
        return $query[1];
    }
    public function Update(){
        $sql = "update ".self::$Tablename." set idDepartamento=\"$this->idDepartamento\",idCiudad=\"$this->idCiudad\",nombre=\"$this->nombre\",referencia=\"$this->referencia\",comentario=\"$this->comentario\" where id=$this->id";
        $query = Executor::doit($sql);
        return $query[1];
    }
