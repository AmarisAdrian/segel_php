<?php
class AnexoUsuarioData{
    public static $Tablename = "anexo_usuario";
    ...
    public function Add(){
        $sql = "insert into ".self::$Tablename."(idUsuario,imagen,comentario)";
        $sql .= "value(\"$this->idUsuario\",\"$this->imagen\",\"$this->comentario\")";
        $query = Executor::doit($sql);
        return $query[1];
    }
    public function Update(){
        $sql = "update ".self::$Tablename." set idUsuario=\"$this->idUsuario\",imagen=\"$this->imagen\",comentario=\"$this->comentario\" where id=$this->id";
        $query = Executor::doit($sql);
        return $query[1];
    }
    ...
    public static function GetByIdUsuario($id){
        ...
        $query = executor::doit($sql);
        return Model::many($query[0],new AnexoUsuarioData());
     }
    public static function GetById($id){
       $sql = "select * from ".self::$Tablename." where id=".$id;
       $query = executor::doit($sql);
       return Model::one($query[0],new AnexoUsuarioData());
    }
           /**
