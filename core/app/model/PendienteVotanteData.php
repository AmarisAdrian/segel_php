<?php
class PendienteVotanteData{
    public static $Tablename = "pendiente_votante";
    ...
    public function Add(){
        $sql = "insert into ".self::$Tablename."(idUsuario,idVotante,estado)";
        $sql .= "value(\"$this->idUsuario\",\"$this->idVotante\",\"$this->estado\")";
        $query = Executor::doit($sql);
        return $query[1];
    }
    public function Update(){
        $sql = "update ".self::$Tablename." set idUsuario=\"$this->idUsuario\",idVotante=\"$this->idVotante\",estado=\"$this->estado\" where id=$this->id";
        $query = Executor::doit($sql);
        return $query[1];
    }
    ...
    public static function GetById($id){
       ...
       $query = executor::doit($sql);
       return Model::one($query[0],new PendienteVotanteData());
    }
           /**
	...
	*/
    public static function Search($q = "", $limit = 8,  $offset = 0, $sort = "nombre", $order = "ASC" ){
        ...
        $query = executor::doit($sql);
        //var_dump($query);die;
        return Model::many($query[0],new PendienteVotanteData());
    }
