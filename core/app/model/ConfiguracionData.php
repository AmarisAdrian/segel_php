<?php
    class ConfiguracionData {
        ...
        public function Add(){
            $sql = "insert into ".self::$Tablename."(nombre,manejador,valor)";
            $sql .= "value(\"$this->nombre\",\"$this->manejador\",\"$this->valor\")";
            $query = Executor::doit($sql);
            return $query[1];
        }
        public function Update(){
            $sql = "update ".self::$Tablename." set \"$this->nombre\",\"$this->manejador\",\"$this->valor\" where id=$this->id";
            $query = Executor::doit($sql);
            return $query[1];
        }
        ...
        public static function GetByHandler($manejador){
            $sql = "select * from ".self::$Tablename." where manejador='".$manejador."'";
            $query = executor::doit($sql);
            return Model::one($query[0],new ConfiguracionData());
        }
          /**
	*   Busqueda, paginacion y ordenamiento
	*   @method ConfiguracionData::Search
