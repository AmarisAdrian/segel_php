...
if(count($_POST)>0){
    ...
    &&!empty($_POST["telefono"])&&!empty($_POST["movil"])&&!empty($_POST["email"])&&!empty($_POST["direccion"])&&!empty($_POST["idDepartamentoPerfil"])&&!empty($_POST["idCiudadPerfil"])){
        $Usuario = UsuarioData::GetById($_POST["id"]);
        ...
        $Usuario->Update();
        Core::alert("Correcto","Usuario actualizado exitosamente","?view=usuario");
    }else{
       core::alert("Error","Ha ocurrido un error","?view=usuario");
    }
}else{
    core::alert("Error","Ha ocurrido un error de validacion","?view=usuario");
}
?>